...
class Cnn_net(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(1, 32, 5, 1, 2), nn.ReLU(), nn.MaxPool2d(2, 2))
        # 第二层内也包含一个卷积层，ReLu激活函数和一个最大池化层
        # 卷积层的输入通道为32，输出通道为64，卷积核大小为5，步长为（1，1），填充值为2；最大池化层和大小为2，步长为2
        self.layer2 = nn.Sequential(
            nn.Conv2d(32, 64, 5, 1, 2), nn.ReLU(), nn.MaxPool2d(2, 2))
        # 设置正则化层
...
def train(x_train, y_train, x_test, y_test, BATCH_SIZE, model, loss_function, optimizer):
    ...
    print("李翰东's procedure has been completed")  # 将XXX改为你的中文名字，最终提交运行截图


def cnn():
    # 加载数据
    x_train, y_train, x_test, y_test = load_data()
    x_train, y_train, x_test, y_test = x_train.to(device), y_train.to(
        device), x_test.to(device), y_test.to(device)
    # 设置训练模型使用批数据的大小
    BATCH_SIZE = 100
    # 加载模型
    model = Cnn_net()
