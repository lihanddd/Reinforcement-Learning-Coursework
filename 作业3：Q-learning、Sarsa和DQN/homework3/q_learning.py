...
n = 4  # size of maze
penalities = 6  # no. of red blocks

# parameters of q learning
alpha = 0.5  # learning rate
gamma = 0.8  # discount factor
epsilon = 1.0
epsilon_delta = 0.0005
...
def step():
    ...
    if new_pos not in terminals:
        # 此处实现Q-learning算法
        Q[current_pos][action] += \
            alpha * (reward[new_pos] + gamma *
                     np.max(Q[new_pos]) - Q[current_pos][action])

    else:
        # 此处实现Q-learning算法
        Q[current_pos][action] += \
            alpha * (reward[new_pos] - Q[current_pos][action])
...
def layout():
    for i in range(n):
        for j in range(n):
            pygame.draw.rect(screen, (255, 255, 255),
                             (j*100, i*100, 100, 100), 0)
            pygame.draw.rect(screen, colors[i]
                             [j], (j*100+3, i*100+3, 94, 94), 0)
            pygame.draw.rect(screen, (255, 255, 255),
                             ((j+n+1)*100, i*100, 100, 100), 0)
            vij = max(Q[(i, j)])
            # vij = min(vij, 1) if vij > 0 else max(vij, -1)
    ...
    pygame.draw.circle(screen, (25, 129, 230),
                       (current_pos[1]*100+50, current_pos[0]*100+50), 30, 0)
...
while running:
    ...
    # print(reward_total)
    # reward_his.append(reward_total)
