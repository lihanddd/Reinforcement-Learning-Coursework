...
def create_everything(seed):
    ...
    OPT_A = torch.optim.Adam(Actor.parameters(),lr=LEARNING_RATE)
    return env, test_env, buf, Q, Qt, Actor, OPT, OPT_A
...
# Update networks
def update_networks(epi, buf, Q, Qt, OPT, OPT_A):
    # Sample a minibatch (s, a, r, s', d)
    # Each variable is a vector of corresponding values
    S, A, R, S2, D = buf.sample(MINIBATCH_SIZE, t)
    prob = Actor(S2)
    #print(prob)
    A2 = torch.multinomial(prob,1)
    ...
    sac_qvalues = q2values - LAMBDA* torch.sum(prob*torch.log(prob),1)
    # If done,
    #   y = r(s, a) + GAMMA * max_a' Q(s', a') * (0)
    # If not done,
    #   y = r(s, a) + GAMMA * max_a' Q(s', a') * (1)
    targets = R + GAMMA * sac_qvalues * (1 - D)
    # Detach y since it is the target. Target values should
    # be kept fixed.
    loss = torch.nn.MSELoss()(targets.detach(), qvalues)

    soft_qvalues = torch.nn.functional.softmax(Qt(S) / LAMBDA)
    prob_S = Actor(S)
    actor_loss = torch.nn.functional.kl_div(prob_S.log(),soft_qvalues ,reduction='sum')
    #actor_loss = torch.nn.functional.kl_div(prob.softmax(dim=-1).log(),soft_qvalues.softmax(dim=-1),reduction='sum')
    # Backpropagation
    OPT.zero_grad()
    loss.backward()
    OPT.step()
...
def train(seed):
    ...
    env, test_env, buf, Q, Qt, Actor, OPT, OPT_A = create_everything(seed)
    ...
    for epi in pbar:
        ...
        if epi >= TRAIN_AFTER_EPISODES:
            ...
            for tri in range(TRAIN_EPOCHS):
                update_networks(epi, buf, Q, Qt, OPT,OPT_A)
...
if __name__ == "__main__":
    ...
    plot_arrays(curves, 'b', 'soft-qlearning')
    plt.legend(loc='best')
    plt.savefig('SAC.png')
    plt.show()